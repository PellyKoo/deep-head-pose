...
def parse_args():
    ...
    parser.add_argument('--dataset', dest='dataset', help='Dataset type.', default='Pose_300W_LP',
                        type=str)
    parser.add_argument('--data_dir', dest='data_dir', help='Directory path for data.',
                        default='/mnt/f/datasets/300W_LP', type=str)
...
def get_fc_params(model):
    # Generator function that yields fc layer params.00
...
if __name__ == '__main__':
    ...
    model = lite_hopenet.shufflenet_v2_x1_0()
    ...
    for epoch in range(num_epochs):
        for i, (images, labels, cont_labels, name) in enumerate(train_loader):
            ...
            if (i+1) % 100 == 0:
                ...
                #        % (epoch+1, num_epochs, i+1, len(pose_dataset)//batch_size, loss_yaw.data[0], loss_pitch.data[0],
                #           loss_roll.data[0]))
                print('Epoch [%d/%d], Step [%d/%d], Yaw Loss: %.4f, Pitch Loss: %.4f, Roll Loss: %.4f'
                      % (epoch + 1, num_epochs, i + 1, len(pose_dataset) // batch_size, loss_yaw.item(), loss_pitch.item(),
                      loss_roll.item()))
